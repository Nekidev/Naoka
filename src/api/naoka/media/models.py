...
class Media(models.Model):
    ...
    mapping = models.CharField(
        max_length=32,
        primary_key=True,
        validators=[validate_mapping],
        help_text=_("Format: MediaType:Provider:ID"),
    )

    type = models.CharField(choices=Type.choices, max_length=12)

    title_ro = models.CharField("Title (Romaji)", max_length=512, blank=True, null=True)
    title_en = models.CharField(
        "Title (English)", max_length=512, blank=True, null=True
    )
    title_es = models.CharField(
        "Title (Spanish)", max_length=512, blank=True, null=True
    )
    title_na = models.CharField("Title (Native)", max_length=512, blank=True, null=True)
    ...
    status = models.CharField(
        choices=Status.choices, default=Status.AUTO, max_length=11
    )
    date_start = models.DateField("Start date", null=True, blank=True)
    date_finish = models.DateField("Finish date", null=True, blank=True)
    ...
    def clean(self) -> None:
        ...
        if not any([self.title_ro, self.title_en, self.title_es, self.title_na]):
        ...
        if not (
            ...
            and all(map(lambda i: isinstance(i, str) and validators.url(i), self.links))
        ):
            raise ValidationError({"links": _("One or more links are not valid.")})

        if self.is_adult is None and self.rating is not None:
            self.is_adult = self.rating in [Media.Rating.R_PLUS, Media.Rating.RX]

    def __str__(self) -> str:
        return self.title_en or self.title_na or self.title_ro or self.title_es


class Mappings(models.Model):
    """
    Links multiple media together.
    """
